...
namespace Predicting_Close_Price_Using_NN {
    ...
    LayerNormLayer::LayerNormLayer(int num_features, double epsilon, double momentum)
        ...
        if (num_features <= 0) {
            throw std::invalid_argument("LayerNormLayer: num_features must be positive.");
        }
    }

    std::vector<double> LayerNormLayer::forward(const std::vector<double>& x_input_sample, bool training_mode) {
        if (static_cast<int>(x_input_sample.size()) != num_features_) {
            throw std::invalid_argument("LayerNormLayer::forward - Input sample size does not match num_features.");
        }
    ...
    std::vector<double> LayerNormLayer::backward(const std::vector<double>& dout_sample, double learning_rate) {
        if (static_cast<int>(dout_sample.size()) != num_features_) {
            throw std::invalid_argument("LayerNormLayer::backward - dout_sample size does not match num_features.");
        }
        ...
        // Fixed: dmean should have positive sign (derivative of (x - mean) w.r.t. mean is -1, but we're computing dL/dmean)
        // The correct formula: dL/dmean = sum(dx_hat * (-inv_stddev)) + dvar * dvar/dmean
        // For layer norm, dvar/dmean = 0 since sum(x - mean) = 0
        double dmean = 0.0;
        for (int j = 0; j < num_features_; ++j) {
            dmean += dx_hat[j] * (-inv_stddev);
        }

        // Gradient of loss w.r.t. input to LayerNorm layer (dx)
